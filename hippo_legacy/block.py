...
class BuildingBlock:
    ...
    def get_purchase_info(self, amount_in_mg, verbosity=1):
        ...
        if self.has_purchase_interpolators:
            return dict(
                price=self._price_interpolator(amount_in_mg),
                lead_time=self._lead_time_interpolator(amount_in_mg),
            )
        else:
            if verbosity:
    ...
    def add_purchase_info(self, **info):
        ...
        if amount in amounts:
            ...
            if info["price"] < existing["price"]:
                self.replace_purchase_info(amount, info)
                mout.warning(
                    f'Preferring ${info["price"]} & {info["lead_time"]}days to ${existing["price"]} & {existing["lead_time"]}days ({self.smiles})'
                )
                return
            else:
                mout.warning(
                    f'Preferring ${existing["price"]} & {existing["lead_time"]}days to ${info["price"]} & {info["lead_time"]}days ({self.smiles})'
                )
                return

            mout.error(
                f'Could not solve duplicate purchase info for {info["amount"]}mg: {self}'
            )
            return
    ...
    def generate_purchase_interpolators(self, debug=False, show=False):
        ...
        if len(amounts) < 2:
            ...
            log_price_interpolator = interp1d(
                log_amounts, log_prices, kind="linear", fill_value="extrapolate"
            )

            self._price_interpolator = PriceInterpolator(
                log_price_interpolator, amounts[0], prices[0]
            )

            self._lead_time_interpolator = interp1d(
                amounts,
                lead_times,
                kind="linear",
                bounds_error=False,
                fill_value=(lead_times[0], lead_times[-1]),
            )
        ...
        if show:
            ...
            trace = go.Scatter(
                name="lead_time", x=amounts, y=lead_times, mode="markers"
            )
            fig.add_trace(trace)
            ...
            trace = go.Scatter(
                name="lead_time (interpolated)", x=xs, y=ys, mode="lines"
            )
            fig.add_trace(trace)
...
class BuildingBlockSet(MutableSet):
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, list):
            return BuildingBlockSet(
                [bb for bb in [self[k] for k in key] if bb is not None]
            )
    ...
    def get_price(self, blanket_amount=None):
        ...
        for bb in self._elements:
            ...
            if bb_price is not None:
                ...
                assert not np.isnan(bb_price), (
                    bb,
                    amount,
                    bb_price,
                    bb.generate_purchase_interpolators(debug=True, show=True),
                )
    ...
    def write(self, directory, bbs_id=None, extra_data=None, verbosity=1):
        ...
        mp.write(
            directory / f"BBS_{identifier:06}.pickle", self, verbosity=verbosity - 1
        )
        ...
        mp.write(
            directory / f"BBS_{identifier:06}.json", json_data, verbosity=verbosity - 1
        )
        ...
        for p in self.products:
            ...
            for i, h in enumerate(p.inspirations):
                ...
                try:
                    ...
                    mout.warning(
                        f"Using hit.ligand_group.rdkit_mol {p.name=} {hit._pose_name=}"
                    )
    ...
    def prepare_for_scoring(self, hit_features):
        ...
        self._hit_feature_coverage = (
            len(hit_features) - num_in_hits_but_not_self
        ) / len(hit_features)

        self._num_new_features = len(self_features - hit_features)

        self._avg_fragmenstein_ddG = np.mean(
            [p._fragmenstein_ddG for p in self.products]
        )
        self._std_fragmenstein_ddG = np.std(
            [p._fragmenstein_ddG for p in self.products]
        )

        p_with_RMSD = [
            p._fragmenstein_mRMSD
            for p in self.products
            if p._fragmenstein_mRMSD is not None
        ]
        if p_with_RMSD:
