...
class HIPPO:
    ...
    def add_protein_reference(self, path):
        ...
        self._protein_system = mp.parsePDB(
            path, verbosity=self.verbosity - 2, alternative_site_warnings=False
        )
    ...
    def add_hit_directory(self, name="hits", path=None, pattern="*-x????_??_bound.pdb"):
        ...
        if len(pdbs) < 1:
            mout.error(
                f"Did not find any PDBs", fatal=True, code="HIPPO.add_hit_data.0"
            )

        self._hit_pdbs = pdbs

        comp_set = CompoundSet.from_bound_pdbs(
            name="hits",
            pdbs=self._hit_pdbs,
    ...
    def add_compounds_from_sdf(self, name, path, idName="name", molColName="mol"):
        ...
        comp_set = CompoundSet.from_df(
            name=name,
            df=mol_df[[idName, molColName]],
            protein=self.protein_system,
    ...
    def add_reaction_products(
        self,
        name,
        metadata,
        data_path,
        mol_pattern,
        skip_unconstrained_minimisation_fail=False,
        expecting=None,
        skip_too_moved=True,
        skip_too_contorted=True,
    ):
        ...
        if self.verbosity:
            mout.success(
                f'Found {self.num_building_blocks} building blocks for "{name}"'
            )
        ...
        for i, mol_file in enumerate(base + mols):
            ...
            if f"{place_type}_fail_minimization_without_constraint" in meta_row:
                compound._minimization_failed_without_constraint = meta_row[
                    f"{place_type}_fail_minimization_without_constraint"
                ].values[0]
            ...
            if "num_atom_difference" in meta_row:
                compound._num_atom_difference = int(
                    meta_row["num_atom_difference"].values[0]
                )
            ...
            if "∆∆G" in meta_row:
                ...
                try:
                    ...
                    compound._fragmenstein_ddG = float(
                        mol_json_data["Energy"]["ligand_ref2015"]["total_score"]
                        - mol_json_data["Energy"]["unbound_ref2015"]["total_score"]
                    )
                    compound._fragmenstein_mRMSD = float(mol_json_data["mRMSD"])
                    compound._fragmenstein_too_moved = (
                        compound._fragmenstein_mRMSD > 1.0
                    )
                    compound._fragmenstein_too_contorted = (
                        compound._fragmenstein_ddG > 0.0
                    )
    ...
    def generate_fingerprints(self, compounds):
        ...
        for i, compound in enumerate(compounds):
            if self.verbosity:
                mout.progress(
                    i,
                    n,
                    prepend="fingerprints",
                    append=f"{mcol.bold}{compound.set_name}{mcol.clear} {compound.name}",
                    fill="#",
                )
        ...
        protein_feature_strs = set(compound.fingerprint.keys()).intersection(
            *[set(c.fingerprint.keys()) for c in fingerprinted_compounds]
        )
    ...
    def score_interaction_coverage(self, show=False):
        ...
        for i, comp_set1 in enumerate(self.compound_sets):
            for comp_set2 in self.compound_sets[i + 1 :]:
                ...
                in_1_but_not_2, in_2_but_not_1 = self.compare_feature_sets(
                    name1, name2, features1, features2
                )
    ...
    def compare_feature_sets(
        self, name1, name2, set1, set2, print_diff=False, verbosity=1
    ):
        ...
        if verbosity:
            ...
            mout.var(
                f"% missed {name1} features", f"{len(in_1_but_not_2)/len(set1):.1%}"
            )
            mout.var(
                f"% covered {name1} features",
                f"{(len(set1)-len(in_1_but_not_2))/len(set1):.1%}",
            )
            if print_diff:
        ...
        if verbosity:
            ...
            mout.var(
                f"% missed {name2} features", f"{len(in_2_but_not_1)/len(set2):.1%}"
            )
            mout.var(
                f"% covered {name2} features",
                f"{(len(set2)-len(in_2_but_not_1))/len(set2):.1%}",
            )
            if print_diff:
    ...
    def random_building_block_set(
        ...
        by_set=False,
        bbs_id=None,
        bb_price_cutoff=None,
        retry_over_budget=50,
    ):
        ...
        if size:
            ...
            if not top_down:
                ...
                if max_lead_time:
                    mout.out(
                        f"Choosing from {len(available)} building blocks up to ${budget} with lead_time <= {max_lead_time} days"
                    )
                else:
                    mout.out(
                        f"Choosing from {len(available)} building blocks up to ${budget}"
                    )
                ...
                while iteration < max_iter:
                    ...
                    if verbosity > 1:
                        ...
                        if max_lead_time:
                            mout.var(
                                ...
                                valCol=(
                                    mcol.error
                                    if this_max_lead_time > max_lead_time
                                    else None
                                ),
                            )
                        else:
                    ...
                    if verbosity > 0:
                        mout.progress(
                            ...
                            budget,
                            prepend="random BBS",
                            append=f"#bbs={len(bb_set)} #prods={len(products)} ${total_price:.2f}",
                        )
                ...
                if not products:
                    ...
                    return self.random_building_block_set(
                        budget=budget,
                ...
                if verbosity:
                    mout.header(
                        f"Choosing BBs up to ${budget} from {len(by_set)} product sets..."
                    )
                ...
                available = CompoundSet(
                    "reminaining_products", self.all_products.compounds
                )
                available.shuffle()
                ...
                if verbosity:
                    if max_lead_time:
                        mout.out(
                            f"Choosing from {len(available)} product compounds up to ${budget} with lead_time <= {max_lead_time} days"
                        )
                        raise NotImplementedError
                    else:
                        mout.out(
                            f"Choosing from {len(available)} product compounds up to ${budget}"
                        )
                ...
                while iteration < max_iter:
                    ...
                    if bb_price_cutoff and any(
                        [
                            bb.get_price(1) > bb_price_cutoff
                            for bb in random_product.building_blocks
                        ]
                    ):
                        continue
    ...
    def fit_umap(self, n_dims=2, plot=True):
        ...
        # ignore metadata
        fingerprint_data = self._umap_df.loc[
            :, ~self._umap_df.columns.isin(self._comp_df_meta_keys)
        ].values
    ...
    def cull_non_purchaseable_building_blocks(self, max_lead_time=None):
        ...
        self._building_blocks = BuildingBlockSet(
            [bb for bb in self.building_blocks if bb.purchaseable]
        )
        ...
        if max_lead_time is not None:
            ...
            self._building_blocks = BuildingBlockSet(
                [
                    bb
                    for bb in self.building_blocks
                    if bb.min_lead_time is not None and bb.min_lead_time < max_lead_time
                ]
            )

            if self.verbosity:
                mout.var(
                    f"#building_blocks (min_lead_time: OK)",
                    f"{len(self.building_blocks)}",
                )
