...
def plot_interaction_punchcard(
    animal, poses, feature_metadata, subtitle=None, opacity=1.0
):
...
def plot_reactant_sankey(animal, subtitle):
    ...
    n_total = len(total)
    n_quoted = len(quoted)
    n_quote_not_attempted = len(
        [bb for bb in total if "quote_attempted" not in bb.tags]
    )
    n_quote_attempted = n_total - n_quote_not_attempted
    n_ok = len([bb for bb in quoted if bb in lead_time and bb in budget])
    # n_bad = len(bad)
    n_expensive = len(
        [bb for bb in quoted if bb not in bad and bb not in budget and bb in lead_time]
    )
    n_slow = len(
        [bb for bb in quoted if bb not in bad and bb in budget and bb not in lead_time]
    )
    n_bad = len(bad)
