...
class HIPPO:
    ...
    * pricing: $USD
    ...
    def add_hits(
        ...
        mout.success(
            f'Loaded {comp_set.num_compounds} compounds as "{comp_set.name}" ({self.hits.num_poses} poses)'
        )
    ...
    def add_elabs(
        ...
        elabs_csv_paths = [
            p
            for p in elabs_csv_paths
            if not any([p.name.startswith(s) for s in elabs_skip_prefix])
        ]
        elabs_csv_paths = [
            p
            for p in elabs_csv_paths
            if not any([s in p.name for s in elabs_skip_substr])
        ]
        elabs_csv_paths = [
            p
            for p in elabs_csv_paths
            if not any([s == p.name for s in elabs_skip_exact])
        ]
        ...
        for j, path in enumerate(elabs_csv_paths):
            ...
            if not (path.parent / placements_output_dir_name).is_dir():
                mout.warning(
                    f'Skipping directory with no subdir named "{placements_output_dir_name}" ({path.name})'
                )
                continue
            ...
            for i, name in enumerate(names):
                ...
                if not comp_output_dir.is_dir():
                    ...
                    if base:
                        ...
                        if (path.parent / "output" / bleach_name).is_dir():
                            ...
                            if (
                                comp_output_dir / f"{bleach_name}{minimised_mol_suffix}"
                            ).exists:
                ...
                pose = Pose.from_mol_and_reference(
                    compound, mol, reference_pdb, site_index=site_index, tags=comp_tags
                )
                ...
                if fstein_json:
                    ...
                    pose._placement_ddG = (
                        fstein_json["Energy"]["bound"]["total_score"]
                        - fstein_json["Energy"]["unbound"]["total_score"]
                    )
        ...
        mout.var(
            "count_base_with_no_success_or_output", count_base_with_no_success_or_output
        )
    ...
    def add_bases(
        self, mol_paths, pdb_paths, metadata_paths, tags=None, overwrite=False
    ):
        ...
        mout.success(
            f'Loaded {comp_set.num_compounds} compounds as "{comp_set.name}" ({self.bases.num_poses} poses)'
        )
    ...
    def random_sample(
        ...
        available_products = CompoundSet(
            "available_products",
            [c for c in self.compounds if c.reactions],
            duplicate="no_check",
        )
        if debug:
        ...
        if self.compound_sets:
            ...
            if tag_stats:
                ...
                for tag in tag_stats:
                    ...
                    mout.out(
                        f'{tag} #compounds={tag_stats[tag]["compounds"]}, #poses={tag_stats[tag]["poses"]}'
                    )
    ...
    def get_compounds(self, tag, search=False, inverse=False, all_compounds=None):
        ...
        if search:
            return CompoundSet(
                f"{tag} in tags (search)",
                [c for c in all_compounds if any([tag in t for t in c.tags])],
            )
        elif inverse:
            assert not search
            return CompoundSet(
                f"{tag} not in tags", [c for c in all_compounds if tag not in c.tags]
            )
        else:
            return CompoundSet(
                f"{tag} in tags", [c for c in all_compounds if tag in c.tags]
            )

    def get_poses(
        self, tag, search=False, inverse=False, all_poses=None, all_compounds=None
    ):
        all_compounds = all_compounds or self.all_compounds
        all_poses = all_poses or self.get_all_poses(all_compounds)
        if search:
    ...
    def plot_interaction_histogram(self, poses, subtitle=None, **kwargs):
        ...
        return plot_interaction_histogram(
            self, poses, FEATURE_METADATA, subtitle, **kwargs
        )
    ...
    def plot_interaction_punchcard(self, poses, subtitle, opacity=1.0, **kwargs):
        ...
        return plot_interaction_punchcard(
            self, poses, FEATURE_METADATA, subtitle, opacity, **kwargs
        )
    ...
    def plot_synthetic_routes(
        self, subtitle=None, cset="elabs", color="num_reactants", **kwargs
    ):
    ...
    def _generate_fingerprints(self, poses):
        ...
        protein_feature_strs = set(pose.fingerprint.keys()).intersection(
            *[set(p.fingerprint.keys()) for p in fingerprinted_poses]
        )
    ...
    def _add_compound_metadata(self, compound, meta_dict, warnings=False):
        ...
        num_reactants = len(
            [k for k in meta_dict if "reactant" in k and "smi" in k]
        )  ### OLD FORMAT
        ...
        if num_reactants == 0:
            ...
            if reactant1.name_is_smiles:
                ...
                for v in [
                    v
                    for k, v in metadata_reactant1.items()
                    if self.reactant_catalog in k
                ]:
            ...
            if reactant2.name_is_smiles:
                ...
                for v in [
                    v
                    for k, v in metadata_reactant2.items()
                    if self.reactant_catalog in k
                ]:
    ...
    def _get_or_create_building_block(self, smiles, debug=False):
        ...
        if smiles in self.building_blocks.smiles:
            if debug:
                mout.debug("returning existing BB")
    ...
    def _get_or_create_compound(self, name, smiles, debug=False, duplicate="error"):
        ...
        if name in self.compounds.names:
            if debug:
                mout.debug("returning existing Compound")
            ...
            if debug:
                mout.debug("returning new Compound")
            return comp
